...
- お寺本や: `src/data/bookshop.ts`
- 公開画像: `src/assets/notices/`、`src/assets/events/`、`src/assets/books/`

サンプル記事はすべて `draft: true` です。公開する記事は日時、場所、写真の掲載許可、代替テキストを確認してから `draft: false` にしてください。終了した行事に関するお知らせは `archived: true` とし、記事を削除せず「過去のお知らせ」に残します。
...
お寺本やページでは、公開用の書名・著者・出版社・税込価格・紹介文・商品ページURLと、法座ごとの在庫冊数を管理します。`/bookshop/` は現在の一覧を表示し、過去の一覧は日付付きカタログIDのURLで残します。入荷数、販売数、仕入価格などの内部情報は公開データへ含めません。特別販売価格は、税込価格の10円・1円の位を切り下げた100円単位で表示します。
