...
- 終了した行事に関する記事は `archived: true` にし、ホームの「最新のお知らせ」から外す。記事とURLは削除せず、お知らせ一覧の「過去のお知らせ」に残す。
- `archived` は公開日だけで推測せず、行事の終了または利用者の明示指示を確認して更新する。

## 8. 行事コンテンツ

行事は `src/content/events/` にMarkdownとして保存する。

「お寺本や」は公開表示の正式名称とし、別表記へ戻さない。既存URLの `/bookshop/`、content ID、内部変数などの互換識別子は表示名と区別して維持する。

- 「お寺本や」の公開カタログは法座ごとに履歴を残し、過去分を新しい在庫で上書きしない。`/bookshop/` は現在分、過去分は固定のカタログID付きURLで公開する。
- 現在のお寺本やページには開催回数と販売記録の累計を表示する。累計更新時も過去カタログの表示は変更せず、販売記録の明細を正本として更新する。
- 入荷・販売記録から法座分を作るときは、元ファイルの合計欄だけでなく明細を再計算した独自集計ファイルも作り、Git管理外の私有領域へ元ファイルとともに保存する。公開PWAには仕入価格などの内部情報を載せない。
...
- 浄泉寺の法座講師の敬称は「氏」ではなく「師」とする。この規則を法座以外の一般人名、著者、寄稿者などへ一括適用しない。
