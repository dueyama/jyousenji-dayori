...
- 通知送信は、初期版では **OneSignal管理画面から人間が手動送信**する。
- サイトの公開と通知送信を必ず分離する。
- 独自ドメインを前提にしない。
...
8. 手動通知用のタイトル・本文・URLを生成するローカルコマンドを作る。これは通知を送信してはならない。
...
- OneSignal REST API Keyを作成・保存・要求しない。
...
## 14. 通知はOneSignal管理画面から手動送信する

初期版では、コード、GitHub Actions、APIから通知を送信してはならない。
...
6. `npm run notification:preview -- <content-id>` で次を生成する。
   ...
   - 短い通知本文
   - 公開済み記事の完全URL
7. 人間がOneSignal管理画面へコピーし、対象と内容を確認して送信する。
...
- 通知本文は短く事実中心にする。
- URLは記事詳細ページへ直接遷移させる。
- 「通知済み」の自動記録や重複送信防止は初期版では実装しない。送信履歴はOneSignal管理画面で確認する。

Codexは、記事を公開しただけで「通知も送信した」と報告してはならない。OneSignal管理画面で人間が送信するまで、通知状態は「未送信」と報告する。
...
- 「通知した」と報告してはならない。初期版の通知は人間がOneSignal管理画面から送信する。
